...
pageUrlRegex = 'property="og:url" content="http:\/\/www.voxnow.de\/(.*?)">'
# ------------------------

def showContent():
	global thisPlugin

	content = getUrl(urlHost+"/sendung_a_z.php")
	match = re.compile(n_regexContent,re.DOTALL).findall(content)
	for m in match:
		if m[1]=="FREE":
			addDirectoryItem(m[2].strip(), {"urlS": "/"+m[4]},m[0])  
			print m
...
def showVideo(urlV, vidN):
	...
	# subDaten
	matchPageUrl = re.compile(pageUrlRegex).findall(content)
	print "-- pageUrl - "+matchPageUrl[0]
	matchIVW = re.compile("ivw=(.*?)&").findall(xmlUrl)
	print matchIVW
	matchTimeType = re.compile("<timetype>(.*?)</timetype>").findall(contentB)
	matchFkContent = re.compile("<fkcontent>(.*?)</fkcontent>").findall(contentB)
	matchSeason = re.compile("<season>(.*?)</season>").findall(contentB)
	matchSession = "2289727260"
	matchUser = "2880224004"
	# subDaten Ende
	...
	print splitted
	# -----
	tcUrl = splitted[0]+"//"+splitted[2]+":1935/"+splitted[3]+"/"
	...
	pageUrl = urlHost+"/"+matchPageUrl[0]
	
	cPlayPath = playPath+"?ivw=/"+matchIVW[0]+"&client=videoplayer&type=content&user=2880224004&session=2289727260&angebot=voxnow&starttime=00:00:00:00&timetype="+matchTimeType[0]+"&fkcontent="+matchFkContent[0]+"&season="+matchSeason[0]
	print "-- swfUrl - "+swfUrl
	print "-- PlayPath - "+cPlayPath
	print "-- app - "+appData
	print "-- tcUrl - "+tcUrl
	#print "-- videoUrl - "+videoUrl
	print "-- pageUrl - "+pageUrl
	
	fullData = tcUrl+' swfVfy=1 playpath='+cPlayPath+' app='+appData+'/ pageUrl='+pageUrl+'/ tcUrl='+tcUrl+' swfUrl='+swfUrl 
	print "------------"
