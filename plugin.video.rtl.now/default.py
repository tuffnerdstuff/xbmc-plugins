...
def showNews(urlN):
	...
	for m in match:
		...
		if m[1]=="FREE":
			zUrl = m[4]
			if zUrl.find("http")!=-1:
				print "-- http on Beginning - "+zUrl
				lastSlash = zUrl.rfind("/")+1
				zUrl = zUrl[lastSlash:]

			if zUrl[0] == "/":
				print "-- slash on Beginning - "+zUrl
				zUrl = zUrl[1:]
				firstSlash = zUrl.find("/")+1
				zUrl = zUrl[:firstSlash]+".php"
			print "-- zUrl - "+zUrl
			addDirectoryItem(m[2].strip(), {"urlS": "/"+zUrl},m[0])  
			print m
	print "--- showNews ok"	
	xbmcplugin.endOfDirectory(thisPlugin)
